...
struct LLVM_CLASS_ABI EdgeCount {
  uint64_t src;
  uint64_t dst;
  uint64_t count;
};
...
LLVM_FUNC_ABI std::vector<uint64_t> computeExtTspLayout(ArrayRef<uint64_t> NodeSizes,
...
LLVM_FUNC_ABI double calcExtTspScore(ArrayRef<uint64_t> Order, ArrayRef<uint64_t> NodeSizes,
                       ArrayRef<uint64_t> NodeCounts,
                       ArrayRef<EdgeCount> EdgeCounts);

/// Estimate the "quality" of the current node order in CFG.
LLVM_FUNC_ABI double calcExtTspScore(ArrayRef<uint64_t> NodeSizes,
                       ArrayRef<uint64_t> NodeCounts,
                       ArrayRef<EdgeCount> EdgeCounts);
...
struct LLVM_CLASS_ABI CDSortConfig {
...
LLVM_FUNC_ABI std::vector<uint64_t> computeCacheDirectedLayout(
