...
LLVM_ABI Error collectPGOFuncNameStrings(ArrayRef<std::string> NameStrs,
...
LLVM_ABI Error readPGOFuncNameStrings(StringRef NameStrings, InstrProfSymtab &Symtab);
...
LLVM_ABI uint64_t ComputeHash(StringRef K);
...
struct LLVM_ABI CountSumOrPercent {
  uint64_t NumEntries;
  double CountSum;
...
// to be output.
struct LLVM_ABI OverlapFuncFilters {
...
struct LLVM_ABI NamedInstrProfRecord : InstrProfRecord {
  StringRef Name;
  uint64_t Hash;
...
template <class IntPtrT> struct LLVM_ABI alignas(8) ProfileData {
