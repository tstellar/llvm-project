...
struct LLVM_ABI YAMLCrossModuleImport {
  StringRef ModuleName;
  std::vector<uint32_t> ImportIds;
};

struct LLVM_ABI SourceLineEntry {
  uint32_t Offset;
  uint32_t LineStart;
  uint32_t EndDelta;
  bool IsStatement;
...
struct LLVM_ABI SourceColumnEntry {
  uint16_t StartColumn;
  uint16_t EndColumn;
};

struct LLVM_ABI SourceLineBlock {
  StringRef FileName;
  std::vector<SourceLineEntry> Lines;
  std::vector<SourceColumnEntry> Columns;
};

struct LLVM_ABI HexFormattedString {
  std::vector<uint8_t> Bytes;
};

struct LLVM_ABI SourceFileChecksumEntry {
  StringRef FileName;
  codeview::FileChecksumKind Kind;
  HexFormattedString ChecksumBytes;
};

struct LLVM_ABI SourceLineInfo {
