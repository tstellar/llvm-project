...
struct LLVM_ABI PreferredTuple {
  LLT Ty;                // The result type of the extend.
  unsigned ExtendOpcode; // G_ANYEXT/G_SEXT/G_ZEXT
  MachineInstr *MI;
};

struct LLVM_ABI IndexedLoadStoreMatchInfo {
  Register Addr;
  Register Base;
  Register Offset;
  bool IsPre;
...
struct LLVM_ABI PtrAddChain {
  int64_t Imm;
  Register Base;
  const RegisterBank *Bank;
};

struct LLVM_ABI RegisterImmPair {
  Register Reg;
  int64_t Imm;
};

struct LLVM_ABI ShiftOfShiftedLogic {
  MachineInstr *Logic;
  MachineInstr *Shift2;
...
struct LLVM_ABI InstructionBuildSteps {
...
struct LLVM_ABI InstructionStepsMatchInfo {
