...
template <class T> struct LLVM_ABI MachineInstrBundleIteratorTraits<T, false> {
...
template <class T> struct LLVM_ABI MachineInstrBundleIteratorTraits<T, true> {
  using list_type = simple_ilist<T, ilist_sentinel_tracking<true>>;
...
template <class T> struct LLVM_ABI MachineInstrBundleIteratorTraits<const T, false> {
  using list_type = simple_ilist<T, ilist_sentinel_tracking<true>>;
  using instr_iterator = typename list_type::const_iterator;
  using nonconst_instr_iterator = typename list_type::iterator;
  using const_instr_iterator = typename list_type::const_iterator;
};
template <class T> struct LLVM_ABI MachineInstrBundleIteratorTraits<const T, true> {
  using list_type = simple_ilist<T, ilist_sentinel_tracking<true>>;
...
template <> struct LLVM_ABI MachineInstrBundleIteratorHelper<false> {
...
template <> struct LLVM_ABI MachineInstrBundleIteratorHelper<true> {
  /// Get the beginning of the current bundle.
  template <class Iterator> static Iterator getBundleBegin(Iterator I) {
...
class LLVM_ABI MachineInstrBundleIterator : MachineInstrBundleIteratorHelper<IsReverse> {
