...
template <typename EnumT> struct LLVM_ABI enum_iteration_traits {
  static constexpr bool is_iterable = false;
};

struct LLVM_ABI force_iteration_on_noniterable_enum_t {
...
template <typename T, bool IsReverse> struct LLVM_ABI SafeIntIterator {
...
template <typename T> struct LLVM_ABI iota_range {
  using value_type = T;
  using reference = T &;
