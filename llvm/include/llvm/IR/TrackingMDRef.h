...
template <class T> class LLVM_ABI TypedTrackingMDRef {
  TrackingMDRef Ref;
...
template <> struct LLVM_ABI simplify_type<TrackingMDRef> {
  using SimpleType = Metadata *;

  static SimpleType getSimplifiedValue(TrackingMDRef &MD) { return MD.get(); }
};

template <> struct LLVM_ABI simplify_type<const TrackingMDRef> {
  using SimpleType = Metadata *;
...
template <class T> struct LLVM_ABI simplify_type<TypedTrackingMDRef<T>> {
  using SimpleType = T *;
...
template <class T> struct LLVM_ABI simplify_type<const TypedTrackingMDRef<T>> {
  using SimpleType = T *;
