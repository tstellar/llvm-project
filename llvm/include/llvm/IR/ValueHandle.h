...
class LLVM_ABI WeakVH : public ValueHandleBase {
public:
...
// dyn_cast, isa, etc.
template <> struct LLVM_ABI simplify_type<WeakVH> {
  using SimpleType = Value *;

  static SimpleType getSimplifiedValue(WeakVH &WVH) { return WVH; }
};
template <> struct LLVM_ABI simplify_type<const WeakVH> {
  using SimpleType = Value *;
...
template <> struct LLVM_ABI DenseMapInfo<WeakVH> {
...
/// changes).
class LLVM_ABI WeakTrackingVH : public ValueHandleBase {
public:
...
// dyn_cast, isa, etc.
template <> struct LLVM_ABI simplify_type<WeakTrackingVH> {
  using SimpleType = Value *;

  static SimpleType getSimplifiedValue(WeakTrackingVH &WVH) { return WVH; }
};
template <> struct LLVM_ABI simplify_type<const WeakTrackingVH> {
  using SimpleType = Value *;
...
template<typename T>
struct LLVM_ABI DenseMapInfo<AssertingVH<T>> : DenseMapInfo<T *> {};
...
template <typename ValueTy> class LLVM_ABI TrackingVH {
...
/// class turns into a trivial wrapper around a pointer.
template <typename ValueTy>
class LLVM_ABI PoisoningVH final
#if LLVM_ENABLE_ABI_BREAKING_CHECKS
    : public CallbackVH
#endif
{
...
template <typename T> struct LLVM_ABI DenseMapInfo<PoisoningVH<T>> {
