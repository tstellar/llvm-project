...
template <typename SubPattern_t> struct LLVM_ABI OneUse_match {
...
template <typename Class> struct LLVM_ABI class_match {
...
struct LLVM_ABI constantexpr_match {
...
/// Inverting matcher
template <typename Ty> struct LLVM_ABI match_unless {
  Ty M;
...
template <typename LTy, typename RTy> struct LLVM_ABI match_combine_or {
  LTy L;
  RTy R;
...
template <typename LTy, typename RTy> struct LLVM_ABI match_combine_and {
  LTy L;
  RTy R;
...
template <int64_t Val> struct LLVM_ABI constantint_match {
  template <typename ITy> bool match(ITy *V) {
...
struct LLVM_ABI cstval_pred_ty : public Predicate {
  template <typename ITy> bool match(ITy *V) {
...
template <typename Predicate> struct LLVM_ABI api_pred_ty : public Predicate {
  const APInt *&Res;
...
template <typename Predicate> struct LLVM_ABI apf_pred_ty : public Predicate {
  const APFloat *&Res;
...
struct LLVM_ABI is_shifted_mask {
...
struct LLVM_ABI is_maxsignedvalue {
...
struct LLVM_ABI is_nonnegative {
...
struct LLVM_ABI is_strictlypositive {
...
struct LLVM_ABI is_nonpositive {
...
struct LLVM_ABI is_negated_power2 {
...
struct LLVM_ABI is_power2_or_zero {
...
struct LLVM_ABI is_lowbit_mask {
...
struct LLVM_ABI icmp_pred_with_threshold {
...
struct LLVM_ABI is_finitenonzero {
...
struct LLVM_ABI is_any_zero_fp {
...
struct LLVM_ABI is_pos_zero_fp {
...
struct LLVM_ABI is_neg_zero_fp {
...
struct LLVM_ABI is_non_zero_fp {
...
template <typename Class> struct LLVM_ABI bind_ty {
  Class *&VR;
...
struct LLVM_ABI specificval_ty {
  const Value *Val;
...
template <typename Class> struct LLVM_ABI deferredval_ty {
  Class *const &Val;
...
/// that value.
struct LLVM_ABI specific_fpval {
  double Val;
...
struct LLVM_ABI bind_const_intval_ty {
  uint64_t &VR;
...
/// value.
template <bool AllowUndefs> struct LLVM_ABI specific_intval {
  APInt Val;
...
struct LLVM_ABI specific_bbval {
  BasicBlock *Val;
...
struct LLVM_ABI AnyBinaryOp_match {
  LHS_t L;
  RHS_t R;
...
//
template <typename OP_t> struct LLVM_ABI AnyUnaryOp_match {
  OP_t X;
...
struct LLVM_ABI BinaryOp_match {
  LHS_t L;
  RHS_t R;
...
template <typename Op_t> struct LLVM_ABI FNeg_match {
  Op_t X;
...
struct LLVM_ABI OverflowingBinaryOp_match {
  LHS_t L;
  RHS_t R;
...
template <typename LHS_t, typename RHS_t, bool Commutable = false>
struct LLVM_ABI SpecificBinaryOp_match
...
struct LLVM_ABI BinOpPred_match : Predicate {
  LHS_t L;
  RHS_t R;
...
//
template <typename SubPattern_t> struct LLVM_ABI Exact_match {
  SubPattern_t SubPattern;
...
template <typename LHS_t, typename RHS_t, typename Class, typename PredicateTy,
          bool Commutable = false>
struct LLVM_ABI CmpClass_match {
...
template <typename T0, unsigned Opcode> struct LLVM_ABI OneOps_match {
  T0 Op1;
...
/// Matches instructions with Opcode and three operands.
template <typename T0, typename T1, unsigned Opcode> struct LLVM_ABI TwoOps_match {
  T0 Op1;
  T1 Op2;
...
struct LLVM_ABI ThreeOps_match {
  T0 Op1;
  T1 Op2;
  T2 Op3;
...
/// Matches shuffle.
template <typename T0, typename T1, typename T2> struct LLVM_ABI Shuffle_match {
  T0 Op1;
  T1 Op2;
  T2 Mask;
...
template <typename Op_t, unsigned Opcode> struct LLVM_ABI CastClass_match {
  Op_t Op;
...
template <typename Op_t> struct LLVM_ABI PtrToIntSameSize_match {
  const DataLayout &DL;
  Op_t Op;
...
struct LLVM_ABI UAddWithOverflow_match {
  LHS_t L;
  RHS_t R;
  Sum_t S;
...
template <typename Opnd_t> struct LLVM_ABI Argument_match {
  unsigned OpI;
  Opnd_t Val;
...
/// Intrinsic matchers.
struct LLVM_ABI IntrinsicID_match {
  unsigned ID;
...
template <typename T0, typename T1, typename T2, typename T3, typename T4,
          typename T5>
struct LLVM_ABI m_Intrinsic_Ty<T0, T1, T2, T3, T4, T5> {
...
template <typename ValTy> struct LLVM_ABI NotForbidUndef_match {
  ValTy Val;
...
template <typename Opnd_t> struct LLVM_ABI Signum_match {
  Opnd_t Val;
...
template <int Ind, typename Opnd_t> struct LLVM_ABI ExtractValue_match {
  Opnd_t Val;
...
template <int Ind, typename T0, typename T1> struct LLVM_ABI InsertValue_match {
  T0 Op0;
  T1 Op1;
...
struct LLVM_ABI VScaleVal_match {
  template <typename ITy> bool match(ITy *V) {
...
struct LLVM_ABI LogicalOp_match {
  LHS L;
  RHS R;
