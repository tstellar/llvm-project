...
struct LLVM_ABI mach_header_64 {
  uint32_t magic;
  uint32_t cputype;
  uint32_t cpusubtype;
  uint32_t filetype;
...
struct LLVM_ABI load_command {
  uint32_t cmd;
  uint32_t cmdsize;
};

struct LLVM_ABI segment_command {
  uint32_t cmd;
  uint32_t cmdsize;
  char segname[16];
  uint32_t vmaddr;
...
struct LLVM_ABI segment_command_64 {
  uint32_t cmd;
  uint32_t cmdsize;
  char segname[16];
  uint64_t vmaddr;
...
struct LLVM_ABI fvmlib_command {
  uint32_t cmd;
  uint32_t cmdsize;
  struct fvmlib fvmlib;
};

struct LLVM_ABI dylib {
  uint32_t name;
  uint32_t timestamp;
...
struct LLVM_ABI dylib_command {
  uint32_t cmd;
  uint32_t cmdsize;
  struct dylib dylib;
};

struct LLVM_ABI sub_framework_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t umbrella;
};

struct LLVM_ABI sub_client_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t client;
};

struct LLVM_ABI sub_umbrella_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t sub_umbrella;
};

struct LLVM_ABI sub_library_command {
  uint32_t cmd;
  uint32_t cmdsize;
...
struct LLVM_ABI prebound_dylib_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t name;
  uint32_t nmodules;
...
struct LLVM_ABI dylinker_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t name;
};

struct LLVM_ABI thread_command {
  uint32_t cmd;
  uint32_t cmdsize;
};

struct LLVM_ABI routines_command {
  uint32_t cmd;
  uint32_t cmdsize;
...
struct LLVM_ABI routines_command_64 {
  uint32_t cmd;
  uint32_t cmdsize;
...
struct LLVM_ABI symtab_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t symoff;
  uint32_t nsyms;
...
struct LLVM_ABI dysymtab_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t ilocalsym;
  uint32_t nlocalsym;
...
struct LLVM_ABI dylib_table_of_contents {
  uint32_t symbol_index;
  uint32_t module_index;
};

struct LLVM_ABI dylib_module {
...
struct LLVM_ABI dylib_module_64 {
  uint32_t module_name;
  uint32_t iextdefsym;
  uint32_t nextdefsym;
  uint32_t irefsym;
...
// The twolevel_hints_command is obsolete and no longer supported.
struct LLVM_ABI twolevel_hint {
  uint32_t isub_image : 8, itoc : 24;
};

// The prebind_cksum_command is obsolete and no longer supported.
struct LLVM_ABI prebind_cksum_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t cksum;
};

struct LLVM_ABI uuid_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint8_t uuid[16];
};

struct LLVM_ABI rpath_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t path;
};

struct LLVM_ABI linkedit_data_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t dataoff;
  uint32_t datasize;
...
struct LLVM_ABI data_in_code_entry {
  uint32_t offset;
  uint16_t length;
  uint16_t kind;
};

struct LLVM_ABI source_version_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint64_t version;
};

struct LLVM_ABI encryption_info_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t cryptoff;
  uint32_t cryptsize;
...
struct LLVM_ABI encryption_info_command_64 {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t cryptoff;
  uint32_t cryptsize;
...
struct LLVM_ABI version_min_command {
...
struct LLVM_ABI build_tool_version {
  uint32_t tool;    // enum for the tool
  uint32_t version; // version of the tool
};

struct LLVM_ABI build_version_command {
...
struct LLVM_ABI dyld_env_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t name;
};

struct LLVM_ABI dyld_info_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t rebase_off;
...
struct LLVM_ABI linker_option_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t count;
};

struct LLVM_ABI fileset_entry_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint64_t vmaddr;
  uint64_t fileoff;
...
struct LLVM_ABI symseg_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t offset;
  uint32_t size;
...
struct LLVM_ABI ident_command {
  uint32_t cmd;
  uint32_t cmdsize;
};

// The fvmfile_command is obsolete and no longer supported.
struct LLVM_ABI fvmfile_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint32_t name;
  uint32_t header_addr;
...
struct LLVM_ABI tlv_descriptor_32 {
  uint32_t thunk;
  uint32_t key;
  uint32_t offset;
};

struct LLVM_ABI tlv_descriptor_64 {
  uint64_t thunk;
  uint64_t key;
  uint64_t offset;
};

struct LLVM_ABI tlv_descriptor {
  uintptr_t thunk;
  uintptr_t key;
  uintptr_t offset;
};

struct LLVM_ABI entry_point_command {
  uint32_t cmd;
  uint32_t cmdsize;
  uint64_t entryoff;
  uint64_t stacksize;
...
struct LLVM_ABI fat_header {
  uint32_t magic;
  uint32_t nfat_arch;
};

struct LLVM_ABI fat_arch {
  uint32_t cputype;
  uint32_t cpusubtype;
  uint32_t offset;
  uint32_t size;
...
struct LLVM_ABI relocation_info {
  int32_t r_address;
  uint32_t r_symbolnum : 24, r_pcrel : 1, r_length : 2, r_extern : 1,
      r_type : 4;
};

struct LLVM_ABI scattered_relocation_info {
...
struct LLVM_ABI any_relocation_info {
  uint32_t r_word0, r_word1;
};

// Structs from <mach-o/nlist.h>
struct LLVM_ABI nlist_base {
  uint32_t n_strx;
  uint8_t n_type;
  uint8_t n_sect;
  uint16_t n_desc;
...
/// load command.
struct LLVM_ABI dyld_chained_fixups_header {
...
struct LLVM_ABI dyld_chained_starts_in_image {
  uint32_t seg_count;
  uint32_t seg_info_offset[1];
};

struct LLVM_ABI dyld_chained_starts_in_segment {
...
// DYLD_CHAINED_IMPORT
struct LLVM_ABI dyld_chained_import {
...
struct LLVM_ABI dyld_chained_import_addend {
  uint32_t lib_ordinal : 8;
  uint32_t weak_import : 1;
  uint32_t name_offset : 23;
  int32_t addend;
...
struct LLVM_ABI dyld_chained_import_addend64 {
...
struct LLVM_ABI dyld_chained_ptr_64_bind {
  uint64_t ordinal : 24;
  uint64_t addend : 8;
...
// DYLD_CHAINED_PTR_64/DYLD_CHAINED_PTR_64_OFFSET
struct LLVM_ABI dyld_chained_ptr_64_rebase {
  uint64_t target : 36;
  uint64_t high8 : 8;
  uint64_t reserved : 7;
  uint64_t next : 12;
...
struct LLVM_ABI x86_thread_state32_t {
  uint32_t eax;
  uint32_t ebx;
  uint32_t ecx;
  uint32_t edx;
...
struct LLVM_ABI x86_thread_state64_t {
  uint64_t rax;
  uint64_t rbx;
  uint64_t rcx;
  uint64_t rdx;
...
struct LLVM_ABI fp_control_t {
  unsigned short invalid : 1, denorm : 1, zdiv : 1, ovrfl : 1, undfl : 1,
      precis : 1, : 2, pc : 2, rc : 2, : 1, : 3;
};

struct LLVM_ABI fp_status_t {
  unsigned short invalid : 1, denorm : 1, zdiv : 1, ovrfl : 1, undfl : 1,
      precis : 1, stkflt : 1, errsumm : 1, c0 : 1, c1 : 1, c2 : 1, tos : 3,
      c3 : 1, busy : 1;
};

struct LLVM_ABI mmst_reg_t {
  char mmst_reg[10];
  char mmst_rsrv[6];
};

struct LLVM_ABI xmm_reg_t {
  char xmm_reg[16];
};

struct LLVM_ABI x86_float_state64_t {
...
struct LLVM_ABI x86_exception_state64_t {
  uint16_t trapno;
  uint16_t cpu;
  uint32_t err;
  uint64_t faultvaddr;
...
struct LLVM_ABI x86_state_hdr_t {
  uint32_t flavor;
  uint32_t count;
};

struct LLVM_ABI x86_thread_state_t {
  x86_state_hdr_t tsh;
  union {
...
struct LLVM_ABI x86_float_state_t {
  x86_state_hdr_t fsh;
  union {
...
struct LLVM_ABI x86_exception_state_t {
  x86_state_hdr_t esh;
  union {
...
struct LLVM_ABI arm_thread_state32_t {
  uint32_t r[13];
  uint32_t sp;
  uint32_t lr;
  uint32_t pc;
...
struct LLVM_ABI arm_thread_state64_t {
  uint64_t x[29];
  uint64_t fp;
  uint64_t lr;
  uint64_t sp;
...
struct LLVM_ABI arm_state_hdr_t {
  uint32_t flavor;
  uint32_t count;
};

struct LLVM_ABI arm_thread_state_t {
  arm_state_hdr_t tsh;
  union {
...
struct LLVM_ABI ppc_thread_state32_t {
  uint32_t srr0;
  uint32_t srr1;
  uint32_t r0;
  uint32_t r1;
...
struct LLVM_ABI ppc_state_hdr_t {
  uint32_t flavor;
  uint32_t count;
};

struct LLVM_ABI ppc_thread_state_t {
  ppc_state_hdr_t tsh;
  union {
...
struct LLVM_ABI CS_CodeDirectory {
...
struct LLVM_ABI CS_BlobIndex {
  uint32_t type;   /* type of entry */
  uint32_t offset; /* offset of entry */
};

struct LLVM_ABI CS_SuperBlob {
